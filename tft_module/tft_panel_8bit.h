...
// Функция для декодирования символа из UTF-8
void decode_utf8(unsigned char* encoded) {
    ...
    if (encoded[0] >> 7 == 0) {
        codepoint = encoded[0];
        num_bytes = 1;
    } else if ((encoded[0] >> 5) == 0x6) {
        codepoint = encoded[0] & 0x1F;
        num_bytes = 2;
    } else if ((encoded[0] >> 4) == 0xE) {
        codepoint = encoded[0] & 0xF;
        num_bytes = 3;
    } else if ((encoded[0] >> 3) == 0x1E) {
        codepoint = encoded[0] & 0x7;
        num_bytes = 4;
    } else {
       // printf("Ошибка: неверный код символа\n");
        return;
    }

    // Декодирование оставшихся байт
    for (int i = 1; i < num_bytes; i++) {
        if ((encoded[i] >> 6) != 0x2) {
           // printf("Ошибка: неверный код символа\n");
            return;
        }
        codepoint = (codepoint << 6) | (encoded[i] & 0x3F);
    }
}
