...
#define RIGHT_DRV_led TIM2->CCR3
#define LEFT_DRV_led  TIM2->CCR4
  ...
  while (1){
	  ...
	  if(send_txPDO1){
	  		   ...
	  		   tx_mailbox.TIR = ((txPDO1 + DC_motor.can_id)<<21)| 0x01; // addr,std0,data0,TXRQ - отправка сообщения
	  			   ...
	  			   tx_mailbox.TDTR = 3;
	  			   tx_mailbox.TDLR = send_data|(DC_motor.in1_drv_pwm << 8)|(DC_motor.in2_drv_pwm << 16);
	  			   tx_mailbox.TDHR = 0;
...
////// IN1_2
				...
				if(subindex != 0){error =ERROR_SUB_INDEX;break;};
				  ...
				  if(cmd == 0x2F){   //save
					  ...
					  if(data&1)DCmotor->in1_drv_pwm = data&3;
					  if(data&2)DCmotor->in2_drv_pwm = data&3;
...
/*---------------  PWM  TIM3 ---------------
	...
	*/
		  ...
		  //TIM2->CCER |= TIM_CCER_CC1P|TIM_CCER_CC2P|TIM_CCER_CC3P|TIM_CCER_CC4P;// output ~low _
			  ...
			  LEFT_DRV_led  = IN1_DRV;//TIM2->CCR3 = ?;
			  RIGHT_DRV_led = IN2_DRV;//TIM2->CCR4 = ?;
